  ...
  try {
    ...
    if (sessionResponse.result && sessionResponse.result.content[0].text) {
      ...
      try {
        ...
        if (sessionData.id) {
          sessionId = sessionData.id; // This is a string UUID, not a number
...
# Set lakehouse as default
spark.sql("USE MCPDemoLakehouse")
print("✅ Using MCPDemoLakehouse as default lakehouse")
`;
